...
//   (endpoint, useToken = false, options = {}) =>
//     request(apiGet, endpoint, useToken, options),
//   [request]
// );
...
export const useApi = () => {
  ...
  // ✅ Updated to support config object with useBearerAuth + options
  const get = useCallback(
    (endpoint, config = {}) => {
      const { useBearerAuth = false, options = {} } = config;
      return request(apiGet, endpoint, useBearerAuth, options);
    },
    [request]
  );

  // ✅ Now includes options (e.g., Basic Auth or headers)
  const post = useCallback(
    (endpoint, body = null, useToken = false, isFormData = false, options = {}) =>
      request(apiPost, endpoint, body, useToken, isFormData, options),
    [request]
